...
import { 
  ...
  StickyNote, FileText, AlertTriangle
} from 'lucide-react';
...
interface RenderDocType extends DocumentType { renderId: string; }
...
const DocTypeEditDialog = ({ open, onOpenChange, mode, formData, setFormData, onSave, onUndo, isChanged, t }: any) => (
    <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent>
            <DialogHeader>
                <DialogTitle>{mode === 'view' ? t('Chi tiết') : mode === 'add' ? t('Thêm mới') : t('Chỉnh sửa')} {t('Loại văn bản')}</DialogTitle>
                <VisuallyHidden><DialogDescription>Nhập thông tin loại văn bản</DialogDescription></VisuallyHidden>
            </DialogHeader>
            <div className="grid gap-4 p-4">
                <div className="space-y-2"><Label className="flex items-center gap-2"><FileText className="h-4 w-4 text-primary" /> {t('Loại văn bản')} *</Label><Input value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} disabled={mode === 'view'} /></div>
...
const AdvancedFilterDialog = ({ open, onOpenChange, filters, setFilters, t }: any) => (
    <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-md p-0 overflow-hidden">
            ...
            <VisuallyHidden><DialogDescription>Lọc danh sách loại văn bản</DialogDescription></VisuallyHidden>

            <ScrollArea className="max-h-[70vh]">
                <div className="p-6 space-y-4">
                    <div className="space-y-2">
                        <Label className="flex items-center gap-2"><FileText className="h-4 w-4 text-primary" /> {t('Loại văn bản')}</Label>
...
export default function DocumentTypesPage() {
    ...
    const collectionRef = useMemo(() => (firestore ? collection(firestore, 'document_types') : null), [firestore]);
    const { permissions, isLoading: permsLoading } = usePermissions('/personnel/document-types');
    const { data: documentTypesData, loading: dataLoading, error: dataError, isOffline } = useCollection<DocumentType>(collectionRef);
    
    const loading = permsLoading || dataLoading;
    const error = dataError;

    const data = useMemo(() => (documentTypesData || []).map((item, idx) => ({ ...item, renderId: `${item.id || 'no-id'}-${idx}` })) as RenderDocType[], [documentTypesData]);

    const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);
    const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
    const [isAdvancedFilterOpen, setIsAdvancedFilterOpen] = useState(false);
    const [selectedItem, setSelectedItem] = useState<RenderDocType | null>(null);
    const [formData, setFormData] = useState<Partial<DocumentType>>({});
    const [initialFormState, setInitialFormState] = useState<Partial<DocumentType>>({});
    ...
    const [currentPage, setCurrentPage] = useLocalStorage('document_types_page_v1', 1);
    const [rowsPerPage, setRowsPerPage] = useLocalStorage('document_types_rows_v1', 10);
    const [columnVisibility, setColumnVisibility] = useLocalStorage<Record<string, boolean>>('document_types_colVis_v1', { name: true, note: true });
    const [filters, setFilters] = useLocalStorage<any>('document_types_filters_v1', {});
    const [sortConfig, setSortConfig] = useLocalStorage<any[]>('document_types_sort_v1', []);
    const [openPopover, setOpenPopover] = useState<string | null>(null);
    const [selectedRowIds, setSelectedRowIds] = useLocalStorage<string[]>('document_types_selected_ids_v1', []);
    const selectedSet = useMemo(() => new Set(selectedRowIds), [selectedRowIds]);

    const isChanged = useMemo(() => JSON.stringify(formData) !== JSON.stringify(initialFormState), [formData, initialFormState]);

    const filteredItems = useMemo(() => data.filter(item => Object.entries(filters).every(([k, v]) => String(item[k as keyof DocumentType] || '').toLowerCase().includes(String(v).toLowerCase()))), [data, filters]);
    ...
    const handleSave = async () => {
        if (!firestore) return;
        const id = (dialogMode === 'edit' || dialogMode === 'view') && selectedItem ? selectedItem.id : (formData.id || doc(collection(firestore, 'document_types')).id);
        await setDoc(doc(firestore, "document_types", id), { ...formData, id }, { merge: true });
    ...
    const processImport = async () => {
        ...
        for (const row of importingData) {
            const id = doc(collection(firestore, 'document_types')).id;
            batch.set(doc(firestore, 'document_types', id), { id, name: String(row['Loại văn bản'] || row['Tên'] || ''), note: String(row['Ghi chú'] || '') }, { merge: true });
        }
    ...
    const handleExport = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "DocumentTypes"); XLSX.writeFile(wb, `DS_LoaiVanBan_${format(new Date(), 'yyyyMMdd')}.xlsx`);
    };

    const columnDefs: any = { name: 'Loại văn bản', note: 'Ghi chú' };
    ...
    return (
        <ClientOnly>
            <TooltipProvider>
                <PageHeader title={t("Bộ danh mục")} description={t("Quản lý loại văn bản")} icon={FileText} />
                <div className="p-4 md:p-6">
                    {/* Cảnh báo offline nếu có */}
                    {isOffline && (
                        <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-center gap-3 text-amber-800 text-sm animate-in fade-in slide-in-from-top-2">
                            ...
                            <p><b>Đang hiển thị dữ liệu ngoại tuyến (Offline):</b> Hệ thống không thể kết nối tới máy chủ Google. Bạn vẫn có thể xem dữ liệu cũ nhưng các thay đổi mới sẽ chỉ được đồng bộ khi có mạng.</p>
                        </div>
                    )}
                    ...
                    <Card>
                        <CardHeader className="py-3 border-b">
                            <div className="flex justify-between items-center">
                                <CardTitle className="text-xl flex items-center gap-2"><FileText className="h-6 w-6 text-primary" />{t('Danh sách Loại văn bản')}</CardTitle>
                                    ...
                                    <TableBody>
                                        {loading ? (
                                            ...
                                            <DataTableEmptyState 
                                                ...
                                                icon={FileText}
                                                title={t('Không tìm thấy loại văn bản')}
                                                filters={filters}
                                                onClearFilters={() => {
                                                    setFilters({});
                                                    setCurrentPage(1);
                ...
                <DocTypeEditDialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen} mode={dialogMode} formData={formData} setFormData={setFormData} onSave={handleSave} onUndo={() => setFormData(initialFormState)} isChanged={isChanged} t={t} />
                ...
                <Dialog open={isImportPreviewOpen} onOpenChange={setIsImportPreviewOpen}>
                    <DialogContent className="sm:max-w-4xl"><DialogHeader><DialogTitle>{t('Xem trước Import')}</DialogTitle><VisuallyHidden><DialogDescription>Dữ liệu chuẩn bị nạp vào hệ thống</DialogDescription></VisuallyHidden></DialogHeader>
                        <ScrollArea className="max-h-[60vh] border rounded-md"><Table><TableHeader><TableRow className="bg-muted/50"><TableHead className="w-[50px] text-center">STT</TableHead><TableHead>{t('Loại văn bản')}</TableHead><TableHead>{t('Ghi chú')}</TableHead></TableRow></TableHeader>
                            <TableBody>{importingData.map((r, i) => (<TableRow key={i}><TableCell className="text-center">{i + 1}</TableCell><TableCell className="font-bold">{String(r['Loại văn bản'] || r['Tên'] || '')}</TableCell><TableCell>{String(r['Ghi chú'] || '')}</TableCell></TableRow>))}</TableBody></Table></ScrollArea>
                ...
                <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Xác nhận xóa?</AlertDialogTitle></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Hủy</AlertDialogCancel><AlertDialogAction className="bg-destructive" onClick={async ()=>{ if(selectedItem && firestore){ await deleteDoc(doc(firestore, "document_types", selectedItem.id)); toast({title: t("Thành công")}); } setIsDeleteDialogOpen(false);}}>Xóa</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
            </TooltipProvider>
        </ClientOnly>
    );
}
