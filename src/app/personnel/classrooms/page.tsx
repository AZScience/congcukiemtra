...
export default function ClassroomsPage() {
    ...
    const { data: classroomsData, loading } = useCollection<Classroom>(classroomsRef);
    const { data: allBuildingBlocks } = useCollection<BuildingBlock>(blocksRef);
    const blockMap = useMemo(() => new Map((allBuildingBlocks || []).map(b => [b.id, b.name])), [allBuildingBlocks]);
    const data = useMemo(() => (classroomsData || []).map((item, idx) => ({ ...item, renderId: `${item.id || 'no-id'}-${idx}` })) as RenderRoom[], [classroomsData]);
                                                ...
                                                <TableRow key={item.renderId} onClick={() => handleRowClick(item.renderId)} data-state={isSelected ? "selected" : ""} className={cn("cursor-pointer odd:bg-white even:bg-muted/20 hover:bg-yellow-300 transition-all", "data-[state=selected]:bg-red-800 data-[state=selected]:text-white")}>
                                                    ...
                                                    {orderedColumns.map(k => (
                                                        <TableCell key={k} className="border-r text-inherit align-middle py-3">
                                                            {k === 'buildingBlockId' ? (blockMap.get(item.buildingBlockId) || item.buildingBlockId) : 
                                                             k === 'hasProjector' ? (item.hasProjector ? t('Có') : t('Không')) :
                                                             k === 'isInactive' ? (item.isInactive ? <Badge variant="destructive">Ngưng</Badge> : <Badge className="bg-green-500">Hoạt động</Badge>) : String(item[k as keyof Classroom] || '')}
                                                        </TableCell>
                                                    ))}
