...
import { 
  ...
  ArrowDown, Filter, X, EllipsisVertical, Save, Building, 
  Undo2, Ban, Eye, FileDown, FileUp, CheckCircle2, ListFilter,
  StickyNote, IdCard, FileText, Activity, Hash
} from 'lucide-react';
...
interface RenderBlock extends BuildingBlock { renderId: string; }
...
const BlockEditDialog = ({ open, onOpenChange, mode, formData, setFormData, onSave, onUndo, isChanged, t }: any) => (
    <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent>
            <DialogHeader>
                <DialogTitle>{mode === 'view' ? t('Chi tiết') : mode === 'add' ? t('Thêm mới') : t('Chỉnh sửa')} {t('Dãy nhà')}</DialogTitle>
                <VisuallyHidden><DialogDescription>Nhập thông tin dãy nhà</DialogDescription></VisuallyHidden>
            </DialogHeader>
            <div className="grid gap-4 p-4">
                <div className="space-y-2"><Label className="flex items-center gap-2"><IdCard className="h-4 w-4 text-primary" /> {t('Mã phòng')} *</Label><Input value={formData.code || ''} onChange={e => setFormData({...formData, code: e.target.value})} disabled={mode === 'view'} /></div>
                <div className="space-y-2"><Label className="flex items-center gap-2"><Building className="h-4 w-4 text-primary" /> {t('Dãy nhà')} *</Label><Input value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} disabled={mode === 'view'} /></div>
                <div className="flex items-center gap-2 space-y-0 py-2"><input type="checkbox" id="isInactive" checked={formData.isInactive || false} onChange={e => setFormData({...formData, isInactive: e.target.checked})} disabled={mode === 'view'} className="h-4 w-4" /><Label htmlFor="isInactive" className="flex items-center gap-2"><Ban className="h-4 w-4 text-destructive" /> {t('Ngưng sử dụng')}</Label></div>
...
const AdvancedFilterDialog = ({ open, onOpenChange, filters, setFilters, t }: any) => (
    <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-lg">
            <DialogHeader><DialogTitle>Bộ lọc nâng cao</DialogTitle><VisuallyHidden><DialogDescription>Lọc danh sách dãy nhà</DialogDescription></VisuallyHidden></DialogHeader>
            <div className="grid gap-4 p-4">
                <div className="space-y-2"><Label className="flex items-center gap-2"><IdCard className="h-4 w-4 text-primary" /> Mã phòng</Label><Input value={filters.code || ''} onChange={e => setFilters({...filters, code: e.target.value})} placeholder="Nhập mã..." /></div>
                <div className="space-y-2"><Label className="flex items-center gap-2"><Building className="h-4 w-4 text-primary" /> Dãy nhà</Label><Input value={filters.name || ''} onChange={e => setFilters({...filters, name: e.target.value})} placeholder="Nhập tên..." /></div>
            </div>
...
export default function BuildingBlocksPage() {
    ...
    const collectionRef = useMemo(() => (firestore ? collection(firestore, 'building-blocks') : null), [firestore]);
    const { data: buildingBlocksData, loading } = useCollection<BuildingBlock>(collectionRef);
    const data = useMemo(() => (buildingBlocksData || []).map((item, idx) => ({ ...item, renderId: `${item.id || 'no-id'}-${idx}` })) as RenderBlock[], [buildingBlocksData]);

    const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);
    const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
    const [isAdvancedFilterOpen, setIsAdvancedFilterOpen] = useState(false);
    const [selectedItem, setSelectedItem] = useState<RenderBlock | null>(null);
    const [formData, setFormData] = useState<Partial<BuildingBlock>>({});
    const [initialFormState, setInitialFormState] = useState<Partial<BuildingBlock>>({});
    ...
    const [currentPage, setCurrentPage] = useLocalStorage('building_blocks_page_v27', 1);
    const [rowsPerPage, setRowsPerPage] = useLocalStorage('building_blocks_rows_v27', 10);
    const [columnVisibility, setColumnVisibility] = useLocalStorage<Record<string, boolean>>('building_blocks_colVis_v27', { code: true, name: true, isInactive: true, note: true });
    const [filters, setFilters] = useLocalStorage<any>('building_blocks_filters_v27', {});
    const [sortConfig, setSortConfig] = useLocalStorage<any[]>('building_blocks_sort_v27', []);
    const [openPopover, setOpenPopover] = useState<string | null>(null);
    const [selectedRowIds, setSelectedRowIds] = useLocalStorage<string[]>('building_blocks_selected_ids_v27', []);
    const selectedSet = useMemo(() => new Set(selectedRowIds), [selectedRowIds]);

    const isChanged = useMemo(() => JSON.stringify(formData) !== JSON.stringify(initialFormState), [formData, initialFormState]);

    const filteredItems = useMemo(() => data.filter(item => Object.entries(filters).every(([k, v]) => String(item[k as keyof BuildingBlock] || '').toLowerCase().includes(String(v).toLowerCase()))), [data, filters]);
    ...
    const openDialog = (mode: DialogMode, item?: any) => {
        ...
        const data = item ? (mode === 'copy' ? { ...item, id: undefined, code: `${item.code}_copy` } : { ...item }) : { code: '', name: '', isInactive: false };
        setFormData(data); setInitialFormState(data); setIsEditDialogOpen(true);
    };

    const handleSave = async () => {
        if (!firestore) return;
        const id = (dialogMode === 'edit' || dialogMode === 'view') && selectedItem ? selectedItem.id : (formData.id || doc(collection(firestore, 'building-blocks')).id);
        await setDoc(doc(firestore, "building-blocks", id), { ...formData, id }, { merge: true });
    ...
    const processImport = async () => {
        ...
        for (const row of importingData) {
            const id = doc(collection(firestore, 'building-blocks')).id;
            batch.set(doc(firestore, 'building-blocks', id), { id, code: String(row['Mã phòng'] || ''), name: String(row['Dãy nhà'] || ''), isInactive: false, note: String(row['Ghi chú'] || '') }, { merge: true });
        }
    ...
    const handleExport = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "BuildingBlocks"); XLSX.writeFile(wb, `DS_DayNha_${format(new Date(), 'yyyyMMdd')}.xlsx`);
    };

    const columnDefs: any = { code: 'Mã phòng', name: 'Dãy nhà', isInactive: 'Trạng thái', note: 'Ghi chú' };
    ...
    return (
        <ClientOnly>
            <TooltipProvider>
                <PageHeader title={t("Bộ danh mục")} description={t("Quản lý dãy nhà")} icon={Building} />
                <div className="p-4 md:p-6">
                    <Card>
                        <CardHeader className="py-3 border-b">
                            <div className="flex justify-between items-center">
                                <CardTitle className="text-xl flex items-center gap-2"><Building className="h-6 w-6 text-primary" />{t('Danh sách Dãy nhà')}</CardTitle>
                                                ...
                                                <TableRow key={item.renderId} onClick={() => handleRowClick(item.renderId)} data-state={isSelected ? "selected" : ""} className={cn("cursor-pointer odd:bg-white even:bg-muted/20 hover:bg-yellow-300 transition-all", "data-[state=selected]:bg-red-800 data-[state=selected]:text-white")}>
                                                    ...
                                                    {orderedColumns.map(k => (
                                                        <TableCell key={k} className="border-r text-inherit align-middle py-3">
                                                            {k === 'isInactive' ? (item.isInactive ? <Badge variant="destructive">Ngưng</Badge> : <Badge className="bg-green-500">Hoạt động</Badge>) : String(item[k as keyof BuildingBlock] || '')}
                                                        </TableCell>
                                                    ))}
                                                    <TableCell className="text-center py-3 text-inherit align-middle">
                                                        <div onClick={e => e.stopPropagation()}>
                                                            <DropdownMenu modal={false}><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="text-primary"><EllipsisVertical className="h-5 w-5"/></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onSelect={()=>openDialog('view', item)}><Eye className="mr-2 h-4 w-4"/>Chi tiết</DropdownMenuItem><DropdownMenuItem onSelect={()=>openDialog('edit', item)}><Edit className="mr-2 h-4 w-4"/>Sửa</DropdownMenuItem><DropdownMenuItem onSelect={()=>openDialog('copy', item)}><Copy className="mr-2 h-4 w-4"/>Sao chép</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem className="text-destructive" onSelect={()=>{ setSelectedItem(item); setIsDeleteDialogOpen(true); }}><Trash2 className="mr-2 h-4 w-4"/>Xóa</DropdownMenuItem></DropdownMenuContent></DropdownMenu>
                                                        </div>
                                                    </TableCell>
                                                </TableRow>
                                            );
                ...
                <BlockEditDialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen} mode={dialogMode} formData={formData} setFormData={setFormData} onSave={handleSave} onUndo={() => setFormData(initialFormState)} isChanged={isChanged} t={t} />

                <Dialog open={isImportPreviewOpen} onOpenChange={setIsImportPreviewOpen}>
                    <DialogContent className="sm:max-w-4xl"><DialogHeader><DialogTitle>Xem trước Import</DialogTitle></DialogHeader>
                        <ScrollArea className="max-h-[60vh] border rounded-md"><Table><TableHeader><TableRow className="bg-muted/50"><TableHead>STT</TableHead><TableHead>Mã phòng</TableHead><TableHead>Dãy nhà</TableHead></TableRow></TableHeader><TableBody>{importingData.map((r, i) => (<TableRow key={i}><TableCell className="text-center">{i+1}</TableCell><TableCell>{String(r['Mã phòng'] || '')}</TableCell><TableCell className="font-bold">{String(r['Dãy nhà'] || '')}</TableCell></TableRow>))}</TableBody></Table></ScrollArea>
                ...
                <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Xác nhận xóa?</AlertDialogTitle></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Hủy</AlertDialogCancel><AlertDialogAction className="bg-destructive" onClick={async ()=>{ if(selectedItem && firestore){ await deleteDoc(doc(firestore, "building-blocks", selectedItem.id)); toast({title: t("Thành công")}); } setIsDeleteDialogOpen(false);}}>Xóa</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
            </TooltipProvider>
        </ClientOnly>
    );
}
