...
import { 
  ...
  Briefcase, Shield, Mail, Phone, MapPin, StickyNote, CloudUpload, CloudDownload, Check, History, ChevronDown
} from 'lucide-react';
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <Select value={filters.position || 'all'} onValueChange={v => setFilters({...filters, position: v === 'all' ? '' : v})}>
                                    ...
                                    <SelectContent>
                                        <SelectItem value="all">Tất cả</SelectItem>
                                        {allPositions?.map((p:any) => <SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>)}
                                    </SelectContent>
                                </Select>
                            </div>
                        </div>
...
export default function EmployeesPage() {
  ...
  const { permissions, isLoading: permsLoading } = usePermissions('/personnel/employees');
  const loading = permsLoading || loadStates.employees;
