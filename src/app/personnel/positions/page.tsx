...
import { 
  ...
  ArrowDown, Filter, X, EllipsisVertical, Save, Briefcase, 
...
interface RenderPos extends Position { renderId: string; }
...
const PosEditDialog = ({ open, onOpenChange, mode, formData, setFormData, onSave, onUndo, isChanged, t }: any) => (
    <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent>
            <DialogHeader>
                <DialogTitle>{mode === 'view' ? t('Chi tiết') : mode === 'add' ? t('Thêm mới') : t('Chỉnh sửa')} {t('Chức vụ')}</DialogTitle>
                <VisuallyHidden><DialogDescription>Nhập thông tin chức vụ</DialogDescription></VisuallyHidden>
            </DialogHeader>
            <div className="grid gap-4 p-4">
                <div className="space-y-2"><Label className="flex items-center gap-2"><Briefcase className="h-4 w-4 text-primary" /> {t('Tên chức vụ')} *</Label><Input value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} disabled={mode === 'view'} /></div>
...
const AdvancedFilterDialog = ({ open, onOpenChange, filters, setFilters, t }: any) => (
    <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-lg">
            <DialogHeader><DialogTitle>Bộ lọc nâng cao</DialogTitle><VisuallyHidden><DialogDescription>Lọc danh sách chức vụ</DialogDescription></VisuallyHidden></DialogHeader>
            <div className="grid gap-4 p-4">
                <div className="space-y-2"><Label className="flex items-center gap-2"><Briefcase className="h-4 w-4 text-primary" /> {t('Tên chức vụ')}</Label><Input value={filters.name || ''} onChange={e => setFilters({...filters, name: e.target.value})} placeholder="Nhập tên..." /></div>
            </div>
...
export default function PositionsPage() {
    ...
    const collectionRef = useMemo(() => (firestore ? collection(firestore, 'positions') : null), [firestore]);
    const { data: positionsData, loading } = useCollection<Position>(collectionRef);
    const data = useMemo(() => (positionsData || []).map((item, idx) => ({ ...item, renderId: `${item.id || 'no-id'}-${idx}` })) as RenderPos[], [positionsData]);

    const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);
    const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
    const [isAdvancedFilterOpen, setIsAdvancedFilterOpen] = useState(false);
    const [selectedItem, setSelectedItem] = useState<RenderPos | null>(null);
    const [formData, setFormData] = useState<Partial<Position>>({});
    const [initialFormState, setInitialFormState] = useState<Partial<Position>>({});
    ...
    const [currentPage, setCurrentPage] = useLocalStorage('positions_page_v27', 1);
    const [rowsPerPage, setRowsPerPage] = useLocalStorage('positions_rows_v27', 10);
    const [columnVisibility, setColumnVisibility] = useLocalStorage<Record<string, boolean>>('positions_colVis_v27', { name: true, note: true });
    const [filters, setFilters] = useLocalStorage<any>('positions_filters_v27', {});
    const [sortConfig, setSortConfig] = useLocalStorage<any[]>('positions_sort_v27', []);
    const [openPopover, setOpenPopover] = useState<string | null>(null);
    const [selectedRowIds, setSelectedRowIds] = useLocalStorage<string[]>('positions_selected_ids_v27', []);
    const selectedSet = useMemo(() => new Set(selectedRowIds), [selectedRowIds]);

    const isChanged = useMemo(() => JSON.stringify(formData) !== JSON.stringify(initialFormState), [formData, initialFormState]);

    const filteredItems = useMemo(() => data.filter(item => Object.entries(filters).every(([k, v]) => String(item[k as keyof Position] || '').toLowerCase().includes(String(v).toLowerCase()))), [data, filters]);
    ...
    const handleSave = async () => {
        if (!firestore) return;
        const id = (dialogMode === 'edit' || dialogMode === 'view') && selectedItem ? selectedItem.id : (formData.id || doc(collection(firestore, 'positions')).id);
        await setDoc(doc(firestore, "positions", id), { ...formData, id }, { merge: true });
    ...
    const processImport = async () => {
        ...
        for (const row of importingData) {
            const id = doc(collection(firestore, 'positions')).id;
            batch.set(doc(firestore, 'positions', id), { id, name: String(row['Tên chức vụ'] || row['Tên'] || ''), note: String(row['Ghi chú'] || '') }, { merge: true });
        }
    ...
    const handleExport = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Positions"); XLSX.writeFile(wb, `DS_ChucVu_${format(new Date(), 'yyyyMMdd')}.xlsx`);
    };

    const columnDefs: any = { name: 'Tên chức vụ', note: 'Ghi chú' };
    ...
    return (
        <ClientOnly>
            <TooltipProvider>
                <PageHeader title={t("Bộ danh mục")} description={t("Quản lý chức vụ")} icon={Briefcase} />
                <div className="p-4 md:p-6">
                    <Card>
                        <CardHeader className="py-3 border-b">
                            <div className="flex justify-between items-center">
                                <CardTitle className="text-xl flex items-center gap-2"><Briefcase className="h-6 w-6 text-primary" />{t('Danh sách Chức vụ')}</CardTitle>
                                        ...
                                        {loading ? <TableRow><TableCell colSpan={orderedColumns.length + 2} className="text-center h-24">{t('Đang tải...')}</TableCell></TableRow> : currentItems.length > 0 ? currentItems.map((item, idx) => {
                                            ...
                                            return (
                                                <TableRow key={item.renderId} onClick={() => handleRowClick(item.renderId)} data-state={isSelected ? "selected" : ""} className={cn("cursor-pointer odd:bg-white even:bg-muted/20 hover:bg-yellow-300 transition-all", "data-[state=selected]:bg-red-800 data-[state=selected]:text-white")}>
                                                    ...
                                                    {orderedColumns.map(k => <TableCell key={k} className="border-r text-inherit align-middle py-3">{String(item[k as keyof Position] || '')}</TableCell>)}
                ...
                <PosEditDialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen} mode={dialogMode} formData={formData} setFormData={setFormData} onSave={handleSave} onUndo={() => setFormData(initialFormState)} isChanged={isChanged} t={t} />
                ...
                <Dialog open={isImportPreviewOpen} onOpenChange={setIsImportPreviewOpen}>
                    <DialogContent className="sm:max-w-4xl"><DialogHeader><DialogTitle>{t('Xem trước Import')}</DialogTitle><VisuallyHidden><DialogDescription>Dữ liệu chuẩn bị nạp vào hệ thống</DialogDescription></VisuallyHidden></DialogHeader>
                        <ScrollArea className="max-h-[60vh] border rounded-md"><Table><TableHeader><TableRow className="bg-muted/50"><TableHead className="w-[50px] text-center">STT</TableHead><TableHead>{t('Tên chức vụ')}</TableHead><TableHead>{t('Ghi chú')}</TableHead></TableRow></TableHeader>
                            <TableBody>{importingData.map((r, i) => (<TableRow key={i}><TableCell className="text-center">{i + 1}</TableCell><TableCell className="font-bold">{String(r['Tên chức vụ'] || r['Tên'] || '')}</TableCell><TableCell>{String(r['Ghi chú'] || '')}</TableCell></TableRow>))}</TableBody></Table></ScrollArea>
                ...
                <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Xác nhận xóa?</AlertDialogTitle></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Hủy</AlertDialogCancel><AlertDialogAction className="bg-destructive" onClick={async ()=>{ if(selectedItem && firestore){ await deleteDoc(doc(firestore, "positions", selectedItem.id)); toast({title: t("Thành công")}); } setIsDeleteDialogOpen(false);}}>Xóa</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
            </TooltipProvider>
        </ClientOnly>
    );
}
