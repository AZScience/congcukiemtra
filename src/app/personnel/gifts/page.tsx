...
const GiftEditDialog = ({ open, onOpenChange, mode, formData, setFormData, onSave, onUndo, isChanged, t }: any) => (
    <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent>
            <DialogHeader>
                <DialogTitle>{mode === 'view' ? t('Chi tiết') : mode === 'add' ? t('Thêm mới') : t('Chỉnh sửa')} {t('Quà tặng')}</DialogTitle>
...
export default function GiftsPage() {
    ...
    const [selectedItem, setSelectedItem] = useState<RenderGift | null>(null);
    const [formData, setFormData] = useState<Partial<GiftType>>({});
    ...
    const [currentPage, setCurrentPage] = useLocalStorage('gifts_page_v27', 1);
    const [rowsPerPage, setRowsPerPage] = useLocalStorage('gifts_rows_v27', 10);
    const [columnVisibility, setColumnVisibility] = useLocalStorage<Record<string, boolean>>('gifts_colVis_v27', { name: true, note: true });
    const [filters, setFilters] = useLocalStorage<any>('gifts_filters_v27', {});
    const [sortConfig, setSortConfig] = useLocalStorage<any[]>('gifts_sort_v27', []);
    const [openPopover, setOpenPopover] = useState<string | null>(null);
    const [selectedRowIds, setSelectedRowIds] = useLocalStorage<string[]>('gifts_selected_ids_v27', []);
    const selectedSet = useMemo(() => new Set(selectedRowIds), [selectedRowIds]);

    const isChanged = useMemo(() => JSON.stringify(formData) !== JSON.stringify(initialFormState), [formData, initialFormState]);

    const filteredItems = useMemo(() => data.filter(item => Object.entries(filters).every(([k, v]) => String(item[k as keyof GiftType] || '').toLowerCase().includes(String(v).toLowerCase()))), [data, filters]);
    const sortedItems = useMemo(() => {
        let items = [...filteredItems];
        if (sortConfig.length > 0) {
            const { key, direction } = sortConfig[0];
            items.sort((a: any, b: any) => direction === 'ascending' ? (a[key] > b[key] ? 1 : -1) : (a[key] < b[key] ? 1 : -1));
        }
        return items;
    ...
    const handleSave = async () => {
        if (!firestore) return;
        const id = (dialogMode === 'edit' || dialogMode === 'view') && selectedItem ? selectedItem.id : (formData.id || doc(collection(firestore, 'gifts')).id);
        await setDoc(doc(firestore, "gifts", id), { ...formData, id }, { merge: true });
    ...
    const processImport = async () => {
        ...
        for (const row of importingData) {
            const id = doc(collection(firestore, 'gifts')).id;
            batch.set(doc(firestore, 'gifts', id), { id, name: String(row['Tên quà tặng'] || row['Tên'] || ''), note: String(row['Ghi chú'] || '') }, { merge: true });
        }
    ...
    const handleExport = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Gifts"); XLSX.writeFile(wb, `DS_QuaTang_${format(new Date(), 'yyyyMMdd')}.xlsx`);
    };

    const columnDefs: any = { name: 'Tên quà tặng', note: 'Ghi chú' };
    ...
    return (
        <ClientOnly>
            <TooltipProvider>
                <PageHeader title={t("Bộ danh mục")} description={t("Quản lý quà tặng")} icon={Gift} />
                <div className="p-4 md:p-6">
                    <Card>
                        <CardHeader className="py-3 border-b">
                            <div className="flex justify-between items-center">
                                <CardTitle className="text-xl flex items-center gap-2"><Gift className="h-6 w-6 text-primary" />{t('Danh mục Quà tặng')}</CardTitle>
                ...
                <GiftEditDialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen} mode={dialogMode} formData={formData} setFormData={setFormData} onSave={handleSave} onUndo={() => setFormData(initialFormState)} isChanged={isChanged} t={t} />
                ...
                <Dialog open={isImportPreviewOpen} onOpenChange={setIsImportPreviewOpen}>
                    <DialogContent className="sm:max-w-4xl"><DialogHeader><DialogTitle>{t('Xem trước Import')}</DialogTitle><VisuallyHidden><DialogDescription>Dữ liệu chuẩn bị nạp vào hệ thống</DialogDescription></VisuallyHidden></DialogHeader>
                        <ScrollArea className="max-h-[60vh] border rounded-md"><Table><TableHeader><TableRow className="bg-muted/50"><TableHead className="w-[50px] text-center">STT</TableHead><TableHead>{t('Tên quà tặng')}</TableHead><TableHead>{t('Ghi chú')}</TableHead></TableRow></TableHeader>
                            <TableBody>{importingData.map((r, i) => (<TableRow key={i}><TableCell className="text-center">{i + 1}</TableCell><TableCell className="font-bold">{String(r['Tên quà tặng'] || r['Tên'] || '')}</TableCell><TableCell>{String(r['Ghi chú'] || '')}</TableCell></TableRow>))}</TableBody></Table></ScrollArea>
