...
import { 
  Truck, PlusCircle, Trash2, Edit, Cog, ChevronLeft, ChevronRight, 
  ...
  GraduationCap, AlertCircle, MessageSquare, MapPin, DoorOpen, Map, Activity, CloudUpload, CloudDownload, Bell
} from 'lucide-react';
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-0">
                    ...
                    <ScrollArea className="max-h-[70vh] col-span-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4">
                            ...
                            <div className="space-y-2"><Label className="flex items-center gap-2"><Landmark className="h-4 w-4 text-primary" /> Đơn vị / Khoa (Chọn nhiều)</Label><MultiSelect options={deptOptions} selected={filters.departments} onChange={(v: any) => setFilters({...filters, departments: v})} placeholder="Chọn khoa..." emptyText="Không có dữ liệu" /></div>
...
const EditDialog = ({ open, onOpenChange, mode, formData: initialFormData, onSave, t, filteredIncidents, rms, lecturers, departments, currentUserEmployee, blocks }: any) => {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-4xl">
                <DialogHeader><DialogTitle>{mode === 'view' ? 'Chi tiết' : (mode === 'edit' ? 'Ghi nhận' : 'Thêm mới')} lớp thực hành ngoài</DialogTitle><VisuallyHidden><DialogDescription>Cấu hình thông tin.</DialogDescription></VisuallyHidden></DialogHeader>
...
export default function ExternalPracticePage() {
    const { t } = useLanguage();
    ...
    const targetRecognition = useMemo(() => 
        recognitions?.find(r => r.name === "Thực hành ngoài"),
    [recognitions]);
    ...
    const externalSchedules = useMemo(() => {
      if (!schedulesData) return [];
      return schedulesData
        .filter(s => (s.building || '').toLowerCase().includes('ngoài') || (s.room || '').toLowerCase().includes('ngoài'))
    ...
    const saveFiltersToCloud = useCallback(async (presetName: string) => {
        ...
        try {
            ...
            await setDoc(doc(firestore, "user_settings", authUser.uid), {
                external_practice_advanced_presets: updatedPresets,
                updatedAt: new Date()
            }, { merge: true });
    ...
    const loadFiltersFromCloud = useCallback(async () => {
        ...
        try {
            ...
            if (docSnap.exists()) {
                ...
                if (data.external_practice_advanced_presets) {
                    setFilterPresets(data.external_practice_advanced_presets);
                } else if (data.external_practice_advanced_filters) {
                    setFilterPresets([{ name: "Bộ lọc cũ", filters: data.external_practice_advanced_filters }]);
                }
            }
        } catch (error) {
    ...
    const [currentPage, setCurrentPage] = useLocalStorage('external_practice_currentPage_v3', 1);
    const [rowsPerPage, setRowsPerPage] = useLocalStorage('external_practice_rows_v3', 10);
    const [sortConfig, setSortConfig] = useLocalStorage<any[]>('external_practice_sortConfig_v3', []);
    const [columnVisibility, setColumnVisibility] = useLocalStorage<Record<string, boolean>>('external_practice_colVis_v4', { 
    ...
    const [filters, setFilters] = useLocalStorage<Partial<Record<keyof DailySchedule, string>>>('external_practice_filters_v3', {});
    const [openPopover, setOpenPopover] = useState<string | null>(null);
    const [selectedRowIds, setSelectedRowIds] = useLocalStorage<string[]>('external_practice_selected_ids_v3', []);
    ...
    const filteredItems = useMemo(() => {
        ...
        return externalSchedules.filter(item => {
            ...
            if ((advancedFilters.buildings?.length || 0) > 0 && !advancedFilters.buildings.includes(item.building)) return false;
            if ((advancedFilters.departments?.length || 0) > 0 && !advancedFilters.departments.includes(item.department)) return false;
            if ((advancedFilters.rooms?.length || 0) > 0 && !advancedFilters.rooms.includes(item.room)) return false;
            if ((advancedFilters.lecturers?.length || 0) > 0 && !advancedFilters.lecturers.includes(item.lecturer)) return false;
    ...
    }, [externalSchedules, filters, advancedFilters]);
    ...
    const handleExport = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "ExternalPractice"); XLSX.writeFile(wb, `DS_ThucHanhNgoai_${format(new Date(), 'yyyyMMdd')}.xlsx`);
    };
    ...
    return (
        <ClientOnly>
            <TooltipProvider>
                <PageHeader title="Thực hành ngoài trường" icon={Truck} />
                <div className="p-4 md:p-6">
                    <Card>
                        <CardHeader className="py-3 border-b">
                            <div className="flex justify-between items-center">
                                <CardTitle className="text-xl flex items-center gap-2"><Truck className="h-6 w-6 text-primary" />Lịch thực hành ngoài</CardTitle>
                                <div className="flex items-center gap-2">
                                    ...
                                    <TableBody>
                                        {schedulesLoading ? (
                                            ...
                                            <DataTableEmptyState 
                                                ...
                                                icon={Truck}
                                                title="Không tìm thấy lịch thực hành ngoài"
