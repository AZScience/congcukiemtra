...
import { 
  Laptop, PlusCircle, Trash2, Edit, Cog, ChevronLeft, ChevronRight, 
...
const EditDialog = ({ open, onOpenChange, mode, formData: initialFormData, onSave, t, filteredIncidents, lecturers, rms, departments, currentUserEmployee, blocks }: any) => {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-4xl">
                <DialogHeader><DialogTitle>{mode === 'view' ? 'Chi tiết' : (mode === 'edit' ? 'Ghi nhận' : 'Thêm mới')} lớp học trực tuyến</DialogTitle><VisuallyHidden><DialogDescription>Cấu hình thông tin.</DialogDescription></VisuallyHidden></DialogHeader>
...
export default function OnlinePage() {
    ...
    const permissions = usePermissions('/monitoring/online');
    ...
    // Find the recognition ID for "Lớp học trực tuyến"
    const targetRecognition = useMemo(() => 
        recognitions?.find(r => r.name === "Lớp học trực tuyến"),
    [recognitions]);
    ...
    const onlineSchedules = useMemo(() => {
      ...
      return schedulesData
        .filter(s => {
          // 1. Chỉ lấy các tiết Trực tuyến
          const isOnline = (s.building || '').toLowerCase().includes('trực tuyến');
          // 2. Loại bỏ các tiết thi
          const isNotExam = s.status !== 'Phòng thi' && s.status !== 'Thi cuối kỳ';
          // 3. LOẠI TRỪ DỰ LIỆU SINH HOẠT (SHCN)
          const content = (s.content || '').toUpperCase();
          const isNotAdvisory = !content.includes("SHCN") && !content.includes("SINH HOẠT");
          
          return isOnline && isNotExam && isNotAdvisory;
        })
        .map((item, idx) => ({ ...item, renderId: `${item.id}-${idx}` })) as any[];
    }, [schedulesData]);

    const [isAdvancedFilterOpen, setIsAdvancedFilterOpen] = useState(false);
    const [advancedFilters, setAdvancedFilters] = useLocalStorage<any>('online_adv_filters_v1', {
    ...
    const saveFiltersToCloud = useCallback(async (presetName: string) => {
        ...
        try {
            ...
            await setDoc(doc(firestore, "user_settings", authUser.uid), {
                online_advanced_presets: updatedPresets,
                updatedAt: new Date()
            }, { merge: true });
    ...
    const loadFiltersFromCloud = useCallback(async () => {
        ...
        try {
            ...
            if (docSnap.exists()) {
                ...
                if (data.online_advanced_presets) {
                    setFilterPresets(data.online_advanced_presets);
                } else if (data.online_advanced_filters) {
                    setFilterPresets([{ name: "Bộ lọc cũ", filters: data.online_advanced_filters }]);
                }
            }
        } catch (error) {
    ...
    const [currentPage, setCurrentPage] = useLocalStorage('online_currentPage_v3', 1);
    const [rowsPerPage, setRowsPerPage] = useLocalStorage('online_rowsPerPage_v3', 10);
    const [sortConfig, setSortConfig] = useLocalStorage<any[]>('online_sortConfig_v3', []);
    const [columnVisibility, setColumnVisibility] = useLocalStorage<Record<string, boolean>>('online_colVis_v4', { 
        date: true, building: true, room: true, period: true, class: true, studentCount: true, attendingStudents: true, lecturer: true, content: true, status: true, type: false, department: false, note: false
    });
    const [filters, setFilters] = useLocalStorage<Partial<Record<keyof DailySchedule, string>>>('online_filters_v3', {});
    const [openPopover, setOpenPopover] = useState<string | null>(null);
    const [selectedRowIds, setSelectedRowIds] = useLocalStorage<string[]>('online_selected_ids_v3', []);
    ...
    const filteredItems = useMemo(() => {
        return onlineSchedules.filter(item => {
    ...
    }, [onlineSchedules, filters, advancedFilters]);
    ...
    const processImport = async () => {
        ...
        for (const row of importingData) {
            ...
            batch.set(doc(firestore, 'schedules', id), { 
                ...
                status: String(row['Trạng thái'] || 'Học trực tuyến') 
            }, { merge: true });
        }
    ...
    const handleExport = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "OnlineClasses"); XLSX.writeFile(wb, `DS_Online_${format(new Date(), 'yyyyMMdd')}.xlsx`);
    };

    const allColumns = ['date', 'building', 'room', 'period', 'type', 'department', 'class', 'studentCount', 'attendingStudents', 'lecturer', 'content', 'status', 'note'];
    const columnDefs: Record<string, string> = { date: 'Ngày', building: 'Dãy nhà', room: 'Phòng', period: 'Tiết', class: 'Lớp', studentCount: 'Sĩ số', attendingStudents: 'SV tham gia', lecturer: 'Giảng viên', content: 'Môn học', status: 'Trạng thái', type: 'LT/TH', department: 'Khoa sử dụng', note: 'Ghi chú' };
    const columnIcons: Record<string, any> = {
        ...
        class: Library,
        studentCount: Users,
        attendingStudents: GraduationCap,
        lecturer: User,
        content: FileText,
        status: Activity,
        type: Layers,
    ...
    return (
        <ClientOnly>
            <TooltipProvider>
                <PageHeader title="Lớp học online" icon={Laptop} />
                <div className="p-4 md:p-6">
                    <Card>
                        <CardHeader className="py-3 border-b">
                            <div className="flex justify-between items-center">
                                <CardTitle className="text-xl flex items-center gap-2"><Laptop className="h-6 w-6 text-primary" />Lịch học trực tuyến</CardTitle>
                                <div className="flex items-center gap-2">
