...
import { 
  BookOpenCheck, PlusCircle, Trash2, Edit, Cog, ChevronLeft, ChevronRight, 
...
import type { DailySchedule, IncidentCategory, Employee, BuildingBlock, Department, Classroom, Lecturer } from '@/lib/types';
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-4xl">
                <DialogHeader><DialogTitle>{mode === 'view' ? 'Chi tiết' : (mode === 'edit' ? 'Ghi nhận' : 'Thêm mới')} lớp thi kết thúc môn</DialogTitle><VisuallyHidden><DialogDescription>Cấu hình thông tin.</DialogDescription></VisuallyHidden></DialogHeader>
                            ...
                            <AccordionItem value="class-info" className="border rounded-lg bg-card px-4">
                                <AccordionTrigger className="hover:no-underline py-3">
                                    <div className="flex items-center gap-2 font-bold text-primary uppercase tracking-wider">
                                        <Library className="h-4 w-4" /> THÔNG TIN PHÒNG THI
                                    </div>
                                </AccordionTrigger>
                                <AccordionContent className="pt-2 pb-4">
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-y-4 gap-x-6 text-sm bg-muted/20 p-4 rounded-lg">
                                        ...
                                        <div className="space-y-1">
                                            <Label className="text-muted-foreground flex items-center gap-2"><School className="h-4 w-4 text-primary" /> Phòng</Label>
                                            {(mode === 'add' || mode === 'copy') ? 
                                        ...
                                        <div className="space-y-1"><Label className="text-muted-foreground flex items-center gap-2"><Users className="h-4 w-4 text-primary" /> Sĩ số</Label>{(mode === 'add' || mode === 'copy') ? <Input type="number" className="h-8" value={formData.studentCount ?? ''} onChange={e => setFormData({...formData, studentCount: Number(e.target.value) || null})} /> : <p className="font-bold">{formData.studentCount || 0}</p>}</div>
                                        <div className="space-y-1">
                                            <Label className="text-muted-foreground flex items-center gap-2"><User className="h-4 w-4 text-primary" /> CBCT 01</Label>
                                            {(mode === 'add' || mode === 'copy') ? <CreatableCombobox options={lecturerOptions} value={formData.proctor1} onChange={(v: any) => setFormData({...formData, proctor1: v})} placeholder="Chọn..." /> : <p className="font-bold">{formData.proctor1}</p>}
                                        </div>
                                        <div className="space-y-1">
                                            <Label className="text-muted-foreground flex items-center gap-2"><User className="h-4 w-4 text-primary" /> CBCT 02</Label>
                                            {(mode === 'add' || mode === 'copy') ? <CreatableCombobox options={lecturerOptions} value={formData.proctor2} onChange={(v: any) => setFormData({...formData, proctor2: v})} placeholder="Chọn..." /> : <p className="font-bold">{formData.proctor2}</p>}
                                        </div>
                                        <div className="space-y-1">
                                            <Label className="text-muted-foreground flex items-center gap-2"><User className="h-4 w-4 text-primary" /> CBCT 03</Label>
                                            {(mode === 'add' || mode === 'copy') ? <CreatableCombobox options={lecturerOptions} value={formData.proctor3} onChange={(v: any) => setFormData({...formData, proctor3: v})} placeholder="Chọn..." /> : <p className="font-bold">{formData.proctor3}</p>}
                                        </div>
                                        <div className="space-y-1">
                                        ...
                                        <div className="md:col-span-2 space-y-1"><Label className="text-muted-foreground flex items-center gap-2"><StickyNote className="h-4 w-4 text-primary" /> Ghi chú</Label><Input value={formData.note || ''} onChange={e => setFormData({...formData, note: e.target.value})} disabled={mode === 'view'} className="h-8" /></div>
                                    </div>
                                </AccordionContent>
                            </AccordionItem>
                                    ...
                                    <AccordionItem value="recording-info" className="border rounded-lg bg-card px-4">
                                        ...
                                        <AccordionContent className="pt-2 pb-4">
                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                <div className="space-y-2"><Label className="flex items-center gap-2"><GraduationCap className="h-4 w-4 text-orange-600" /> Sinh viên dự thực tế</Label><Input type="number" value={formData.attendingStudents ?? ''} onChange={e => setFormData({...formData, attendingStudents: e.target.value ? Number(e.target.value) : null})} disabled={mode === 'view'} /></div>
                                                <div className="space-y-2">
                                                    <Label className="flex items-center gap-2"><AlertCircle className="h-4 w-4 text-orange-600" /> Việc phát sinh</Label>
...
export default function ExamsPage() {
    ...
    const saveFiltersToCloud = useCallback(async (presetName: string) => {
        ...
        try {
            ...
            await setDoc(doc(firestore, "user_settings", authUser.uid), {
                exams_advanced_presets: updatedPresets,
                updatedAt: new Date()
            }, { merge: true });
    ...
    const loadFiltersFromCloud = useCallback(async () => {
        ...
        try {
            ...
            if (docSnap.exists()) {
                ...
                if (data.exams_advanced_presets) {
                    setFilterPresets(data.exams_advanced_presets);
                } else if (data.exams_advanced_filters) {
                    setFilterPresets([{ name: "Bộ lọc cũ", filters: data.exams_advanced_filters }]);
                }
            }
        } catch (error) {
    ...
    const [currentPage, setCurrentPage] = useLocalStorage('exams_currentPage_v3', 1);
    const [rowsPerPage, setRowsPerPage] = useLocalStorage('exams_rows_v3', 10);
    const [sortConfig, setSortConfig] = useLocalStorage<any[]>('exams_sortConfig_v3', []);
    const [columnVisibility, setColumnVisibility] = useLocalStorage<Record<string, boolean>>('exams_colVis_v3', { 
        date: true, building: true, room: true, period: true, type: true, department: true, class: true, studentCount: true, proctor1: true, proctor2: true, proctor3: true, content: true, status: true, note: false
    });
    const [filters, setFilters] = useLocalStorage<Partial<Record<keyof DailySchedule, string>>>('exams_filters_v3', {});
    const [openPopover, setOpenPopover] = useState<string | null>(null);
    const [selectedRowIds, setSelectedRowIds] = useLocalStorage<string[]>('exams_selected_ids_v3', []);
    ...
    const filteredItems = useMemo(() => {
        return examSchedules.filter(item => {
            ...
            if (advancedFilters.lecturers.length > 0) {
                const itemLecs = [item.proctor1, item.proctor2, item.proctor3].filter(Boolean);
    ...
    }, [examSchedules, filters, advancedFilters]);
    ...
    const openDialog = (mode: DialogMode, item?: any) => {
        ...
        if (mode === 'edit' || mode === 'add' || mode === 'copy') {
            ...
            if (!data.employee) data.employee = currentUserEmployee?.nickname || currentUserEmployee?.name || '';
        }
    ...
    const ensureReferences = async (data: any) => {
        ...
        const namesToCheck = [data.proctor1, data.proctor2, data.proctor3].filter(Boolean);
    ...
    const processImport = async () => {
        ...
        for (const row of importingData) {
            ...
            batch.set(doc(firestore, 'schedules', id), { 
                ...
                proctor1: String(row['CBCT 01'] || row['Giảng viên'] || ''),
    ...
    const handleExport = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "ExamSchedules"); XLSX.writeFile(wb, `DS_LichThi_${format(new Date(), 'yyyyMMdd')}.xlsx`);
    };
    ...
    return (
        <ClientOnly>
            <TooltipProvider>
                <PageHeader title="Thi kết thúc môn" icon={BookOpenCheck} />
                <div className="p-4 md:p-6">
                    <Card>
                        <CardHeader className="py-3 border-b">
                            <div className="flex justify-between items-center">
                                <CardTitle className="text-xl flex items-center gap-2"><BookCheck className="h-6 w-6 text-primary" />Lịch thi cuối kỳ</CardTitle>
                                <div className="flex items-center gap-2">
                ...
                <Dialog open={isImportPreviewOpen} onOpenChange={setIsImportPreviewOpen}>
                    <DialogContent className="sm:max-w-4xl"><DialogHeader><DialogTitle>Xem trước Import</DialogTitle></DialogHeader>
                        <ScrollArea className="max-h-[60vh] border rounded-md"><Table><TableHeader><TableRow className="bg-muted/50"><TableHead>STT</TableHead><TableHead>Ngày</TableHead><TableHead>Phòng</TableHead><TableHead>Lớp</TableHead><TableHead>CBCT</TableHead></TableRow></TableHeader><TableBody>{importingData.map((r, i) => (<TableRow key={i}><TableCell className="text-center">{i+1}</TableCell><TableCell>{String(r['Ngày'] || '')}</TableCell><TableCell className="font-bold">{String(r['Phòng'] || '')}</TableCell><TableCell>{String(r['Lớp'] || '')}</TableCell><TableCell>{String(r['CBCT 01'] || r['Giảng viên'] || '')}</TableCell></TableRow>))}</TableBody></Table></ScrollArea>
