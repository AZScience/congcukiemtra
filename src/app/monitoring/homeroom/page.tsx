...
import { 
  ...
  ListFilter, Check, ChevronsUpDown, Library, Clock, CalendarDays, Camera,
  Landmark, Users, Hash, Layers, GraduationCap, AlertCircle, 
  MessageSquare, User, Map, School, FileText, StickyNote, DoorOpen, Activity, CloudUpload, CloudDownload, Loader2, Bell, History, ChevronDown
} from 'lucide-react';
...
import type { DailySchedule, IncidentCategory, Employee, BuildingBlock, Classroom, Lecturer, Department } from '@/lib/types';
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-4xl">
                <DialogHeader><DialogTitle>{mode === 'view' ? 'Chi tiết' : (mode === 'edit' ? 'Ghi nhận' : 'Thêm mới')} lớp sinh hoạt chủ nhiệm</DialogTitle><VisuallyHidden><DialogDescription>Cấu hình thông tin.</DialogDescription></VisuallyHidden></DialogHeader>
...
export default function HomeroomPage() {
    ...
    const { permissions } = usePermissions('/monitoring/homeroom') as any;
    
    // Memoize references to avoid infinite loops and unnecessary loading states
    const [isAdvancedFilterOpen, setIsAdvancedFilterOpen] = useState(false);
    const [advancedFilters, setAdvancedFilters] = useLocalStorage<any>('homeroom_adv_filters_v1', {
    ...
    const schedulesRef = useMemo(() => {
        ...
        // Chỉ tải dữ liệu của ngày đang chọn để tăng tốc độ load trang và đạt real-time tối ưu
    ...
    // Find the recognition ID for "Cố vấn học tập" (Homeroom)
    const targetRecognition = useMemo(() => 
        recognitions?.find(r => r.name === "Cố vấn học tập"),
    [recognitions]);
    ...
    const homeroomSchedules = useMemo(() => {
      ...
      return schedulesData
        .filter(s => {
          ...
          return keywords.some(k => content.includes(k) || status.includes(k));
        })
    ...
    const saveFiltersToCloud = useCallback(async (presetName: string) => {
        ...
        try {
            ...
            await setDoc(doc(firestore, "user_settings", authUser.uid), {
                homeroom_advanced_presets: updatedPresets,
                updatedAt: new Date()
            }, { merge: true });
    ...
    const loadFiltersFromCloud = useCallback(async () => {
        ...
        try {
            ...
            if (docSnap.exists()) {
                ...
                if (data.homeroom_advanced_presets) {
                    setFilterPresets(data.homeroom_advanced_presets);
                } else if (data.homeroom_advanced_filters) {
                    setFilterPresets([{ name: "Bộ lọc cũ", filters: data.homeroom_advanced_filters }]);
                }
            }
        } catch (error) {
    ...
    const [currentPage, setCurrentPage] = useLocalStorage('homeroom_currentPage_v3', 1);
    const [rowsPerPage, setRowsPerPage] = useLocalStorage('homeroom_rowsPerPage_v3', 10);
    const [sortConfig, setSortConfig] = useLocalStorage<any[]>('homeroom_sortConfig_v3', []);
    const [columnVisibility, setColumnVisibility] = useLocalStorage<Record<string, boolean>>('homeroom_colVis_v3', { 
    ...
    const [filters, setFilters] = useLocalStorage<Partial<Record<keyof DailySchedule, string>>>('homeroom_filters_v3', {});
    const [openPopover, setOpenPopover] = useState<string | null>(null);
    const [selectedRowIds, setSelectedRowIds] = useLocalStorage<string[]>('homeroom_selected_ids_v3', []);
    ...
    const filteredItems = useMemo(() => {
        ...
        return homeroomSchedules.filter(item => {
    ...
    }, [homeroomSchedules, filters, advancedFilters]);
    ...
    const handleExport = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Homeroom"); XLSX.writeFile(wb, `DS_SinhHoatCVHT_${format(new Date(), 'yyyyMMdd')}.xlsx`);
    };
    ...
    return (
        <ClientOnly>
            <TooltipProvider>
                <PageHeader title="Cố vấn học tập" icon={BookUser} />
                <div className="p-4 md:p-6">
                    <Card>
                        <CardHeader className="py-3 border-b">
                            <div className="flex justify-between items-center">
                                <CardTitle className="text-xl flex items-center gap-2"><BookUser className="h-6 w-6 text-primary" />Lịch sinh hoạt chủ nhiệm</CardTitle>
                                <div className="flex items-center gap-2">
                                        ...
                                        {(schedulesLoading && currentItems.length === 0) ? (
                                            ...
                                            return (
                                                <TableRow key={item.renderId} onClick={() => handleRowClick(item.renderId)} data-state={isSelected ? "selected" : ""} className={cn("cursor-pointer odd:bg-white even:bg-muted/30 transition-all hover:bg-yellow-300 hover:text-black", "data-[state=selected]:bg-red-800 data-[state=selected]:text-white")}>
                                                    ...
                                                    <TableCell className="w-16 p-0 text-center border-l border-blue-100 sticky right-0 z-20 bg-white group-hover:bg-yellow-300 shadow-[-2px_0_5px_rgba(0,0,0,0.05)] align-middle">
                                                        <Tooltip>
                                                            <TooltipTrigger asChild>
                                            ...
                                            <DataTableEmptyState 
                                                ...
                                                icon={BookUser}
                                                title="Không tìm thấy lịch sinh hoạt"
