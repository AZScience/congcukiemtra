...
import { PanelLeft, PanelRight, ChevronsLeft, ChevronsRight, Menu } from "lucide-react"
  ...
  (
    ...
    return (
      <aside
        ref={ref}
        className={cn(
          "group hidden md:flex flex-col text-card-foreground h-svh bg-card transition-[width] duration-300 ease-in-out",
...
>(({ className, onClick, ...props }, ref) => {
  ...
  if (isMobile) {
    return (
      <Button
        ...
        variant="ghost"
        size="icon"
        className={cn("h-10 w-10 text-muted-foreground hover:text-foreground hover:bg-muted", className)}
        onClick={(event) => {
          onClick?.(event);
          toggleSidebar();
        }}
  ...
  if (asChild) {
    return (
        <Slot onClick={(event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
          onClick?.(event);
          toggleSidebar();
        }} {...rest}>
            {props.children}
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <Button
          ...
          variant="ghost"
          size="icon"
          className={cn("h-10 w-10 text-primary hover:text-primary/90 hover:bg-muted", className)}
          onClick={(event) => {
            onClick?.(event);
            toggleSidebar();
          }}
...
>(({ className, ...props }, ref) => (
  <ul
    ...
    className={cn(
      "mx-4 flex min-w-0 translate-x-px flex-col gap-1 border-l-2 border-muted pl-5 py-1",
      "group-data-[collapsible=icon]:hidden",
      className
    )}
    {...props}
...
const SidebarMenuSubItem = React.forwardRef<
  HTMLLIElement,
  React.ComponentProps<"li">
>(({ ...props }, ref) => <li ref={ref} className="w-full relative before:content-[''] before:absolute before:w-2 before:border-b-2 before:border-muted before:left-[-21px] before:top-1/2 before:-translate-y-1/2" {...props} />)
...
>(({ asChild, size = "md", isActive, className, as: Comp = 'button', tooltip, children, ...props }, ref) => {
  const { isMobile, state } = useSidebar();
  const buttonProps = {
    ref,
  ...
  const button = asChild ? <Slot {...buttonProps}>{children}</Slot> : <Comp {...buttonProps}>{children}</Comp>;
